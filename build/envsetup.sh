...
cat <<EOF
Additional SuperiorOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- superiorgerrit:   A Git wrapper that fetches/pushes patch from/to SuperiorOS Gerrit Review.
- superiorrebase:   Rebase a Gerrit change and push it again.
- superiorremote:   Add git remote for SuperiorOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cloremote:       Add git remote for matching CodeLinaro repository.
- githubremote:    Add git remote for SuperiorOS Github.
...
function breakfast()
{
    target=$1
    local variant=$2
    local aosp_target_release=$(cat ${ANDROID_BUILD_TOP}/vendor/superior/vars/aosp_target_release 2>/dev/null)
...
function superiorremote()
{
    ...
    local SUPERIOR_USER=$(git config --get review.review.superioros.org.username)
    if [ -z "$SUPERIOR_USER" ]
    then
        git remote add superior ssh://review.superioros.org:29418/$PFX$PROJECT
    else
        git remote add superior ssh://$SUPERIOR_USER@review.superioros.org:29418/$PFX$PROJECT
    fi
...
function githubremote()
{
    ...
    git remote add github http://github.com/Project-Superior/$PROJECT
...
function makerecipe() {
    ...
    then
        pwd
        superiorremote
        git push superior HEAD:refs/heads/'$1'
    fi
    '
}

function superiorgerrit() {
    ...
    local user=`git config --get review.review.superioros.org.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "superiorgerrit" ]; then
...
function superiorrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        ...
        echo "      superiorrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.superioros.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
